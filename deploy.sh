...
then
  graph deploy --product hosted-service universexyz/daosubgraph
  # Remove manifest
  rm subgraph.yaml
  exit 0
elif [ "$GRAPH" = "mainnet-remote" ]
then
  graph deploy --studio universexyz
  # Remove manifest
  rm subgraph.yaml
  exit 0
fi
...
{
  graph create universexyz/DAO-Governance-Graph --node ${GRAPH_NODE}
} || {
  echo 'Subgraph was already created'
}

# Deploy subgraph
graph deploy universexyz/DAO-Governance-Graph --ipfs ${IPFS_NODE} --node ${GRAPH_NODE}
